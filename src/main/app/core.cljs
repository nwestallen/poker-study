...
(defpage PageContent
    (d/div
           (d/h1 {:class-name "text-4xl font-bold py-5"} "♠" (d/span {:class "text-red-600"} "♥︎ ") "Understanding Poker Theory " (d/span {:class "text-sky-600"} "♦︎") (d/span {:class "text-green-500"}"♣︎"))
      (d/h2 {:id "hand-rankings" :class-name (:h2 outline-style)} "Hand Rankings")
           (d/h3 {:id "suba" :class-name (:h3 outline-style)} "Subsection A")
           (d/p {:class-name "indent-6"}"I'm gonna put some more text here")
