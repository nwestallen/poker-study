...
(defnc HeightInput [{:keys [height set-height]}]
  (d/div {:class-name (css :flex :flex-col :p-1 :rounded-md :text-white :shadow-lg :text-xs {:background-color "rgb(64 64 64)"})}
         (d/p {:class-name (css :text-center)} "Range")
         (d/input {:type "text" :class-name (css :w-full :text-center :text-white {:background-color "rgb(64 64 64)"}) :value height :on-change (fn [e] (set-height (js/parseInt e.target.value)))})))
...
(defnc MixSlider [{:keys [mix set-mix]}]
  ...
  (d/div {:class-name (css :w-full :relative :shadow-lg)}
   (d/svg {:xlmns "http://www.w3.org/2000/svg" :viewBox "0 0 100 12" :width "100%" :height "14px" :preserveAspectRatio "none"}
...
(defnc HeightSlider [{:keys [height set-height]}]
  (d/input {:type "range" :class-name (css :shadow-lg {:writing-mode "vertical-lr"} {:direction "rtl"} {:vertical-align "bottom"} {:-webkit-appearance "none"} {:appearance "none"} {:background "rgb(64 64 64)"} {:width "14px"}
                                           ["&::-webkit-slider-thumb" {:-webkit-appearance "none"} {:background-color "lightgrey"} {:width "14px"} {:height "14px"} :cursor-pointer]) :on-change (fn [e] (set-height (js/parseInt e.target.value))) :value height}))
