...
(defnc ActionForm [{:keys [on-submit actions set-actions!]}]
  (d/form {:class-name (css :h-fit {:width "35%"} :my-3 :mr-3 :rounded-lg :p-2 {:background-color "rgb(175 175 175)"}) :on-submit #(do (.preventDefault %) (.log js/console actions) (on-submit actions))}
