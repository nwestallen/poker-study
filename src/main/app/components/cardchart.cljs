...
(defnc Cardsquare [{:keys [hand strategy set-strategy update-strat paint hide] {:keys [raise call fold]} :act}]
  (let [
...
(defnc PureChart [{:keys [strategy set-strategy update-strat hidelist]}]
  (let [[paint set-paint] (hooks/use-state false)]
    (d/div {:class-name (css :grid {:grid-template-columns "repeat(14, 7%)"} {:gap "1px"} {:width "fit-content" :height "100%"}) :on-mouse-down #(set-paint true) :on-mouse-up #(set-paint false)}
           ...
           (for [p (partition 13 strategy)] (<> ($ Cardsquare {:hand (second (name (key (first p)))) :strategy strategy :set-strategy set-strategy :update-strat update-strat :paint paint}) (for [i p] ($ Cardsquare {:act (val i) :hand (name (key i)) :strategy strategy :set-strategy set-strategy :update-strat update-strat :paint paint :hide (contains? hidelist (key i))})))))
    ))
