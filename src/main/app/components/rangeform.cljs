...
(defnc RangeForm [{:keys [on-submit]}]
  ...
  (d/form {:class-name (css :h-fit :w-full :mb-3 :rounded-lg :p-2 {:background-color "rgb(175 175 175)"}) :on-submit #(do (.preventDefault %) (.log js/console (prn-str state)) (on-submit state))}
